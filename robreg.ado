...
void rr_q_r2(real colvector y, real colvector w, struct rr scalar S)
{
    ...
    if (S.cons & S.p==0) {
        ...
        r = rr_resid(y, J(rows(y),0,.), S.b0, 1, S.scale0)
        if (S.f.scale!=0) {
            S.sdev0 = Q.sdev()
    ...
    real scalar smax, struct _rr_irls scalar S)
{
    if (smax>=.) return(1)
    return(editmissing(
        quadcross(w, mm_biweight_rho(r / smax, S.k))/S.wsum // = mean()
        * (S.N / S.df), 0) <= S.delta)
}
    ...
    struct rr scalar S)
{
    ...
    if (w==1) S.f.scale = sqrt(cross(1, r[q]:^2)       / (rows(q)        - p))
    else      S.f.scale = sqrt(cross(1, w[q], r[q]:^2) / (cross(1, w[q]) - p))
